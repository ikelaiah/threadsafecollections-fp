...
## Limits on manual locking

`Lock` returns an `ILockToken` that releases the lock when it leaves scope. Do
not hold a manual token and then call public methods on the same collection:
public methods acquire the same lock themselves and non-reentrant
implementations (notably on POSIX) deadlock. See
...
- Reentrant lock acquisition (manual locking with public calls).
