...
[API cheat sheet](../start/cheat-sheet.md)
...
the Linux CI package-smoke job. Benchmarks are run locally, not in CI. The
maintained target platforms are **Windows x86-64** and **Linux**; macOS and
other targets are out of scope for now.
...
in [Thread-safety, iteration, and lock policy](../guides/thread-safety-and-iteration.md):
...
The default output is `docs/../start/cheat-sheet.md`. To check generation without
overwriting it:
...
`examples/` is listed in the `docs/../project/building.md` inventory (and vice versa),
and that `docs/../start/cheat-sheet.md` matches a fresh generator run. Compilability of
